import { genreLabel } from "./genre-label.ts";
import { letterBucketInSentence } from "./letter-buckets.ts";
...
/**
 * "ACTION" and "action" both become "Action", and "RPG" stays "RPG".
 *
 ...
 * a listing page has `req.params.genre`, which is whatever the address says.
 * Neither is a label. This used to title-case both by hand, which turned the
 * one initialism in the enum into "Rpg Games" in the BreadcrumbList of every
 * RPG page; utils/genre-label.ts is the one place that spelling is decided.
 */
const titleCase = genreLabel;
...
export function buildBreadcrumbs(
  ...
  if (letter) {
    // Named the way the page's own title names it — "'D'" for a letter, "a
    // number" for the "0-9" page, which upper-cased like a letter read as a
    // range of characters. See utils/letter-buckets.ts.
    crumbs.push({
      name: `Games starting with ${letterBucketInSentence(letter)}`,
