...
/**
 * Postgres SQLSTATE 22021, character_not_in_repertoire: its answer to a text
 * parameter that carries a NUL byte ("invalid byte sequence for encoding
 * "UTF8": 0x00"). A text column cannot hold U+0000 at all.
 */
const CHARACTER_NOT_IN_REPERTOIRE = "22021";

/**
 * Whether an error is Postgres refusing a string it cannot store — in
 * practice, one with a NUL in it.
 *
 * Nothing upstream of the queries rejects U+0000, and nothing should have to:
 * it arrives from "%00" in a path segment, a search term, a developer name, a
 * login e-mail or a comment, and every one of those reaches a bound parameter
 * sooner or later. Postgres refusing it is correct; what was wrong was the
 * answer. The error carried no HTTP status, so app.ts reported it as a 500 and
 * wrote a full stack trace to error.log — for a request any anonymous client
 * can repeat as often as it likes, which is precisely the noise that buries a
 * real fault. It is the client's malformed input, so it is a 400.
 */
export function isUnstorableTextError(error: unknown): boolean {
  return violation(error)?.code === CHARACTER_NOT_IN_REPERTOIRE;
}
