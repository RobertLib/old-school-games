 ...
 * Location header.
 *
 * Everything after the *first* "?", not the text between the first and the
 * second. A "?" is legal inside a query string, and `split("?")[1]` dropped
 * the rest at the next one: "/Action?search=who?&page=2" was redirected to
 * "/action?search=who", losing the page — exactly the silent rewrite the
 * paragraph above rules out.
 */
export function rawQuery(req: { originalUrl: string }): string {
  const start = req.originalUrl.indexOf("?");
  const query = start === -1 ? "" : req.originalUrl.slice(start + 1);
