...
describe("sanitizeHtml", () => {
  ...
  /**
   * The style="" attribute, which is the half the policy in app.ts explicitly
   * does *not* refuse: "style-src-attr" is left at 'unsafe-inline' for the
   * three hundred style attributes the templates are built from, so for stored
   * markup this sanitizer is the only barrier there is.
   *
   * DOMPurify keeps the attribute by default and only sanitizes the CSS in it,
   * so none of these is an XSS — they are the things declarations alone can do:
   * cover the page, hide text from a reader but not a crawler, and fetch from
   * somebody else's server on every view.
   */
  it.each([
    ["an overlay over the whole page", 'position: fixed; inset: 0'],
    ["text hidden from the reader", "opacity: 0"],
    ["an off-site background request", "background: url(https://evil.test/x)"],
  ])("strips %s", (_label, declaration) => {
    const html = sanitizeHtml(`<p style="${declaration}">Text</p>`);
