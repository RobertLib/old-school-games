  ...
  return {
    ...
    timers: {
      setTimeout: vi.fn((handler: () => void, ms: number) => {
        // An id of its own, because toHaveBeenCalledWith compares by value:
        // two bare { unref } objects sharing the one spy are equal, so an
        // assertion about *which* timer was cleared passed for any of them —
        // the fallback and the idle sweep were indistinguishable.
        const timer = { unref, id: scheduled.length };
...
function harness() {
  ...
  // The session store, whose only part of the contract used here is close().
...
describe("createShutdown", () => {
  ...
  describe("on a signal", () => {
    ...
    /**
     * connect-pg-simple starts a prune timer when it is built and nothing used
     * to stop it. Its prunes query the very pool being ended here, so one
     * firing in between ran against a pool that had gone — reported through
     * the store's errorLog as a failure on the way out of a perfectly clean
     * deploy, and under test it is a handle nothing releases.
     */
    it("closes the session store before ending the pool", () => {
      const h = harness();
      ...
      expect(h.store.close.mock.invocationCallOrder[0]!).toBeLessThan(
        h.pool.end.mock.invocationCallOrder[0]!,
      );
    });

    // Draining must not be stoppable by a store that objects to being closed
    // — twice, or at all. The pool still has to be ended and the process still
    // has to leave.
    it("drains anyway when the store refuses to close", () => {
      const h = harness();

      h.store.close.mockImplementation(() => {
  ...
  describe("on a second signal", () => {
    it("does not re-run the drain", () => {
      ...
      h.shutdown("SIGTERM");
      const scheduled = h.clock.scheduled.length;
      h.shutdown("SIGTERM");

      expect(h.server.close).toHaveBeenCalledTimes(1);
      expect(h.server.closeIdleConnections).toHaveBeenCalledTimes(1);
      expect(h.stopTimers).toHaveBeenCalledTimes(1);
      // The first drain's fallback and its idle sweep, and nothing new.
      expect(h.clock.scheduled).toHaveLength(scheduled);
    ...
    // The other order. The guard above used to return before it looked at the
    // code it was handed, so an uncaught exception on the way out of a deploy
    // was logged and then reported to the supervisor as a clean stop.
    it("lets a crash during a signal's drain raise the exit code", () => {
      const h = harness();
  ...
  /**
   * One closeIdleConnections() at the start fixed only the sockets idle at
   * that moment. A keep-alive socket busy when SIGTERM arrived answered with
   * "Connection: keep-alive" and then sat idle for keepAliveTimeout — five
   * seconds, the same as the fallback — so under any traffic the drain never
   * finished first: every deploy ended in "exceeded", exit 1 and a pool that
   * was never ended. Reproduced against a real http.Server before the fix.
   */
  describe("the idle sweep", () => {
    it("keeps closing idle sockets until the listener has closed", () => {
      const h = harness();

      h.shutdown("SIGTERM");

      const sweep = h.clock.scheduled[1]!;

      expect(sweep.ms).toBe(IDLE_SWEEP_MS);

      sweep.handler();
      h.clock.scheduled[2]!.handler();

      expect(h.server.closeIdleConnections).toHaveBeenCalledTimes(3);
      expect(h.clock.scheduled).toHaveLength(4);

      h.server.drained();

      // The sweep scheduled last is the one still pending, and it goes.
      expect(h.clock.clearTimeout).toHaveBeenCalledWith(
        h.clock.scheduled[3]!.timer,
      );
    });

    it("is not started for a server with nothing to sweep", () => {
      const server = fakeServer();
      const clock = fakeTimers();
    ...
    it("stops when the process leaves", () => {
      const h = harness();
  ...
  describe("the force-exit fallback", () => {
    it("is scheduled for the shutdown timeout and unref'd", () => {
      ...
      // wait for. The idle sweep below is unref'd for the same reason, so
      // every timer the drain schedules is.
      expect(h.clock.unref).toHaveBeenCalledTimes(h.clock.scheduled.length);
    });
    ...
    /**
     * It used to be cancelled the moment the listener closed, which left
     * pool.end() — which waits for every checked-out client — with no bound
     * at all: a query still running at SIGTERM could stretch the drain towards
     * the platform's SIGKILL. It now stays armed until the pool has closed.
     */
    it("still bounds the pool once the listener has closed", () => {
      const h = harness();

      h.shutdown("SIGTERM");
      h.server.drained();

      expect(h.clock.clearTimeout).not.toHaveBeenCalledWith(
        h.clock.scheduled[0]!.timer,
      );
    ...
    // A pool that closes after the fallback fired must not be a second exit.
    it("leaves only once when the pool closes after it fired", () => {
      const h = harness();
