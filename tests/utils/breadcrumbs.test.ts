...
describe("buildBreadcrumbs", () => {
  ...
  // The one page of the A–Z browse that is not a letter — see
  // utils/letter-buckets.ts. Upper-cased like a letter it read "Games
  // starting with '0-9'", a range of characters rather than what the titles
  // on it have in common. Named the way the page's own <title> names it.
  it("calls the digits' page by what its titles start with", () => {
    expect(trail(buildBreadcrumbs({ letter: "0-9" }))).toEqual([
      "Home -> /",
      "Games starting with a number -> (current)",
    ]);
  });
