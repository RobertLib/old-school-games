...
describe("PostgresRateLimitStore", () => {
  ...
  describe("pruning", () => {
    ...
    it("sweeps expired windows on the way in and on the interval", async () => {
      ...
      // Once on the way in — machines here are stopped when idle and rarely
      // live ten minutes, so a sweep that waited for the interval rarely ran
      // at all — and again on every interval after that.
      vi.useFakeTimers();
      startRateLimitPruning();

      expect(sweep, "no sweep ran on start").toBeDefined();
      await sweep;

      sweep = undefined;
      await vi.advanceTimersByTimeAsync(PRUNE_INTERVAL_MS);

      expect(sweep, "no sweep ran on the interval").toBeDefined();
      await sweep;
    ...
    it("starts one timer however many times it is asked", async () => {
      ...
      // One sweep on start, one on the interval; the repeat calls add none.
    ...
    it("stops sweeping once told to", async () => {
      ...
      // The sweep on start has already run; nothing follows it.
      expect(query).toHaveBeenCalledTimes(1);
    ...
    it("can be started again after being stopped", async () => {
      ...
      // A sweep per start, then one on the interval the second start armed.
      expect(query).toHaveBeenCalledTimes(2);
    ...
    it("keeps sweeping after a failure", async () => {
      ...
      // The sweep on start is the one that failed; the two intervals after
      // it still ran.
      expect(query).toHaveBeenCalledTimes(3);
    });
  });
