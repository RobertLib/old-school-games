...
describe("News Validations", () => {
  describe("validateNews", () => {
    ...
    /**
     * The raw text is not what gets stored. Content made only of markup the
     * sanitiser removes passed the emptiness test and was published as an
    ...
    ])("refuses content that is only %s", (_label, content) => {
      const errors = validateNews({ title: "Valid title", content });
    ...
    // A picture is something a reader sees, so an article that is only an
    // image is a real article.
    it("accepts an article that is only an image", () => {
      const errors = validateNews({
        title: "Valid title",
        content: '<p><img src="/images/new-games.png" alt="New games"></p>',
      });
