...
describe("Game rating ranking", () => {
  ...
  /**
   * "Year ▼" — and /?orderBy=release, which defaults to DESC off the filtered
   * pages — sorted with a bare `"release" DESC`, and Postgres puts NULLs
   * *first* in a descending sort. A game with no year recorded is not the
   * newest game, but every one of them led the listing: with a tenth of the
   * catalogue undated, the whole first page was games with no year at all.
   */
  describe("ordering by year", () => {
    const seedYears = async () => {
      const ids: Record<string, number> = {};

      for (const [title, release] of [
        ["Undated A", null],
        ["Doom", 1993],
        ["Undated B", null],
        ["Quake", 1996],
        ...
        ids[title] = (await Game.create({ title, genre: "ACTION", release }))
          .id;
      }
    ...
    it("puts the undated games last in a descending sort", async () => {
      await seedYears();

      const games = await Game.find({ orderBy: "release", orderDir: "DESC" });

      expect(games.map((game) => game.release)).toEqual([
        1996, 1994, 1994, 1993, null, null,
      ]);
    });

    it("puts them last in an ascending one, as it always did", async () => {
      await seedYears();

      const games = await Game.find({ orderBy: "release", orderDir: "ASC" });

      expect(games.map((game) => game.release)).toEqual([
        1993, 1994, 1994, 1996, null, null,
      ]);
    });

    // Off the filtered pages the direction defaults to DESC, which is the
    // address the finding came in on.
    it("does the same for the default direction and a page of results", async () => {
      await seedYears();

      const first = await Game.find({ orderBy: "release", limit: 3, page: 1 });
      const second = await Game.find({ orderBy: "release", limit: 3, page: 2 });

      expect(first.map((game) => game.release)).toEqual([1996, 1994, 1994]);
      expect(second.map((game) => game.release)).toEqual([1993, null, null]);
    });

    // The id still breaks ties, in the sort's own direction, among the games
    // sharing a year and among the undated ones alike.
    it("keeps the id tie-break for a shared year and for no year", async () => {
      const ids = await seedYears();
    ...
    // A letter page sorts inside the "letterGames" CTE, a different statement
    // from the one above, and takes the same ordering clause.
    it("puts them last on a letter page too", async () => {
      await Game.create({ title: "Dune", genre: "ACTION", release: null });
      await Game.create({ title: "Doom", genre: "ACTION", release: 1993 });
      ...
      expect(games.map((game) => game.release)).toEqual([1993, null]);
    });
  });
});
