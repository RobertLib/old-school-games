...
describe("GameOfTheWeek Model", () => {
  ...
  /**
   * Driven through getOrSelectCurrent rather than by calling the selector.
   *
   * selectNewGameOfTheWeek is private now and takes the lock-holding client:
   * it used to be public with the pool as its default executor, so the easiest
   * way to call it was also the one that skipped the advisory lock and could
   * insert a second pick for the week. These cases had been doing exactly
   * that, which is how a test suite ends up covering a path the application
   * must never take. The statements asserted on are the same; they are
   * checked on the client, because that is who runs them.
   */
  describe("picking a new game of the week", () => {
    /** getCurrent finds nothing, then re-reads whatever was inserted. */
    function noCurrentPick(reread: unknown[] = []) {
      (mockDb.query as any)
        .mockResolvedValueOnce({ rows: [] })
        .mockResolvedValueOnce({ rows: reread });
    }
    ...
    it("should select a new game of the week from eligible games", async () => {
      ...
      (client.query as any)
        ...
        .mockResolvedValueOnce({ rows: [{ id: 123 }] }) // eligible game
        .mockResolvedValueOnce({ rows: [{ id: 2, gameId: 123 }] }) // insert
      ...
      // actually returns.
      const [eligibilitySql] = (client.query as any).mock.calls[3];
    ...
    it("should select any game when no eligible games found", async () => {
      ...
      (client.query as any)
        ...
        .mockResolvedValueOnce({ rows: [] }) // advisory lock
        .mockResolvedValueOnce({ rows: [] }) // nobody else picked yet
        .mockResolvedValueOnce({ rows: [] }) // nothing eligible
        .mockResolvedValueOnce({ rows: [{ id: 456 }] }) // the fallback
        .mockResolvedValueOnce({ rows: [{ id: 3, gameId: 456 }] }) // insert
        .mockResolvedValueOnce({ rows: [] }); // COMMIT
      ...
      const [fallbackSql] = (client.query as any).mock.calls[4];
    ...
    it("should insert nothing when no games exist at all", async () => {
      const client = mockPoolClient();
      ...
      // BEGIN, the lock, the "has anyone picked yet" check, the eligibility
      // query and the fallback — and then nothing, because there was nothing
      // to insert. The COMMIT is the sixth.
      expect(client.query).toHaveBeenCalledTimes(6);
      expect(
        (client.query as any).mock.calls.some(([sql]: [string]) =>
          sql.includes('INSERT INTO "game_of_the_week"'),
        ),
      ).toBe(false);
      expect(result).toBeNull();
