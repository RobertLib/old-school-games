...
vi.mock("../../db", () => ({
  default: { query: vi.fn() },
  // models/game.ts derives its suggestion threshold from this at import time
  // (Math.min against it), so a mock without it fails the whole file at
  // module load with "No export is defined on the mock" rather than in any
  // one test. The value is irrelevant here; nothing below searches.
...
describe("write-through cache invalidation", () => {
   ...
   *
   * The curated lists were the next one left behind, and the reason the key
   * moved into utils/sidebar-cache.ts: the middleware named that entry with a
   * literal, so nothing in models/game.ts had a name to delete. Deleting the
   * last game of a genre left the sidebar linking a list page that answers
   * 404. See NON_EMPTY_LISTS_KEY.
   */
