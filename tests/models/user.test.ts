...
describe("User Model", () => {
  ...
  describe("upsertAdmin", () => {
    ...
    // A reset has to end what the old password let in: a session opened with
    // a phished password used to survive the reset meant to lock it out. The
    // behaviour is exercised against the real table in
    // tests/models/user-sessions.test.ts; this pins it to the one statement,
    // so the new password and the sign-out cannot happen one without the
    // other.
    it("ends the account's sessions in the statement that resets it", async () => {
      (mockDb.query as any).mockResolvedValueOnce({
        rows: [{ id: 1, created: false }],
      });
      ...
      expect(sql).toContain(`"sess" -> 'user' ->> 'id'`);
    });
