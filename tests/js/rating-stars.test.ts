...
/**
 * Which game a star rates.
 *
 * Every star said "Rate 3 out of 5" and nothing else, so a listing page — a
 * card per game, five stars on each — offered a screen reader two dozen
 * identical sets, and a list of the page's buttons (VoiceOver's rotor,
 * NVDA's elements list) could not say which game any of them belonged to.
 * The views pass the title on data-title; the stars carry it in their own
 * labels, and a role="group" named after the game holds the five.
 */
describe("rating-stars.js — the stars name their game", () => {
  async function mount(attrs: Record<string, string>) {
    const el = document.createElement("rating-stars");
  ...
  const labels = (el: Element) =>
    Array.from(el.shadowRoot!.querySelectorAll(".star")).map((star) =>
      star.getAttribute("aria-label"),
    );

  it("puts the title in every star's label", async () => {
    const el = await mount({ "data-title": "Doom" });

    expect(labels(el)).toEqual([1, 2, 3, 4, 5].map((n) => `Rate ${n} out of 5 for Doom`));
  });

  // The number leads, as it does in the untitled label, so a game with a
  // number in its name cannot be misheard as the rating.
  it("keeps the rating ahead of a title that ends in a number", async () => {
    const el = await mount({ "data-title": "Doom 2" });

    expect(labels(el)[2]).toBe("Rate 3 out of 5 for Doom 2");
  });

  it("holds the five in a group named after the game", async () => {
    const el = await mount({ "data-title": "Doom" });
    const group = el.shadowRoot!.querySelector(".stars")!;

    expect(group.getAttribute("role")).toBe("group");
    expect(group.getAttribute("aria-label")).toBe("Rate Doom");
    expect(group.querySelectorAll(".star")).toHaveLength(5);
  });

  /**
   * The title is data from the database. Written into the markup string, one
   * with a quote in it would have closed the attribute and turned the rest of
   * itself into elements inside the shadow root.
   */
  it("treats the title as text, never as markup", async () => {
    const title = 'The "Secret" <b>of</b> Monkey Island';
    const el = await mount({ "data-title": title });

    expect(labels(el)[0]).toBe(`Rate 1 out of 5 for ${title}`);
    expect(el.shadowRoot!.querySelector("b")).toBeNull();
    expect(el.shadowRoot!.querySelectorAll(".star")).toHaveLength(5);
  });

  // A vote re-renders the stars from scratch; the names must come back with
  // them.
  it("keeps the names after a vote re-renders the stars", async () => {
    (global as any).fetch = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ averageRating: 4, ratingCount: 2, userRating: 4, ratings: {} }),
    });
    vi.spyOn(window, "alert").mockImplementation(() => {});

    const el = await mount({ "data-title": "Doom" });

    el.shadowRoot!.querySelectorAll<HTMLElement>(".star")[3]!.click();
    ...
    expect(labels(el)[3]).toBe("Rate 4 out of 5 for Doom");
    expect(el.shadowRoot!.querySelector(".stars")!.getAttribute("aria-label")).toBe(
      "Rate Doom",
    );
  });

  // Nothing to name them after: the labels the stars always had, and no
  // unnamed group announced around them.
  it("keeps the old labels when no title is given", async () => {
    const el = await mount({});

    expect(labels(el)).toEqual([1, 2, 3, 4, 5].map((n) => `Rate ${n} out of 5`));
    expect(el.shadowRoot!.querySelector(".stars")!.hasAttribute("role")).toBe(false);
  });

  // The carousel's stars sit inside a link and are not controls at all.
  it("leaves read-only stars unlabelled", async () => {
    const el = await mount({ "data-title": "Doom", readonly: "true" });

    expect(labels(el)).toEqual([null, null, null, null, null]);
    expect(el.shadowRoot!.querySelector(".stars")!.hasAttribute("role")).toBe(false);
  });

  /**
   * The views are where the title comes from, so each one that renders a
   * widget a visitor can vote with has to pass it — through <%= %>, which
   * escapes it. Read-only widgets are exempt: they carry no labels.
   */
  it("is given the title by every view that renders an interactive widget", () => {
    const root = path.resolve(__dirname, "../..");
    const tags = globSync("views/**/*.ejs", { cwd: root }).flatMap((file) =>
      [
        ...readFileSync(path.join(root, file), "utf-8").matchAll(
          // Quoted values whole, since each one holds an EJS tag with a ">".
          /<rating-stars\b(?:[^>"]|"[^"]*")*>/g,
        ),
      ].map((match) => ({ file, tag: match[0] })),
    );

    const interactive = tags.filter(({ tag }) => !/readonly="true"/.test(tag));

    expect(interactive.length).toBeGreaterThan(0);

    for (const { file, tag } of interactive) {
      expect(tag, file).toMatch(/\bdata-title="<%= [\w.]+\.title %>"/);
    }
  });
});
