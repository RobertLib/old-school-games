...
/**
 * The cover as the two inline backgrounds name it — the hero and the
 * click-to-play poster — which EJS's HTML escaping cannot protect inside a CSS
 * url(), so the template escapes it for CSS itself.
 */
describe("the cover inside a CSS url()", () => {
  const backgrounds = async (cover: string) => {
    const { document } = new JSDOM(await renderDetail(cover)).window;

    return [".game-hero-background", ".game-detail-poster"].map((selector) =>
      document.querySelector<HTMLElement>(selector)!.getAttribute("style"),
    );
  };

  /**
   * The escaping used to start with encodeURI over the whole value, which
   * escapes "%" itself: an already-encoded ".../Space%20Quest.png" became
   * ".../Space%2520Quest.png", a file that does not exist, in exactly the
   * backgrounds the preload above exists for.
   */
  it("leaves an address that is already encoded as it is", async () => {
    const cover = "https://media.example/covers/Space%20Quest%20I.png";

    for (const style of await backgrounds(cover)) {
      expect(style).toBe(`background-image: url('${cover}')`);
    }
  });

  it("escapes what would end the quoted url() early", async () => {
    const cover = "https://media.example/it's (1).png";

    for (const style of await backgrounds(cover)) {
      expect(style).toBe(
        "background-image: url('https://media.example/it%27s%20%281%29.png')",
      );
    }
  });
});
