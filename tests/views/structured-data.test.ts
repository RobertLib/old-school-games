...
/**
 * The A–Z browse's one page that is not a letter: every game whose slug
 * begins with a digit, at "/letter/0-9". Upper-cased like a letter it would
 * have called itself "Games Starting with '0-9'" — a range of characters, not
 * what its titles have in common — so the route names it, and the heading,
 * the breadcrumb schema and the canonical all have to agree about where and
 * what it is.
 */
describe("the digits' letter page", () => {
  function renderDigits(): Promise<string> {
    return ejs.renderFile(
      path.join(VIEWS, "index.ejs"),
      {
        ...APP_LOCALS,
        games: [game(1, "1942"), game(2, "688 Attack Sub")],
        total: 2,
        page: 1,
        limit: 25,
        letter: "0-9",
        letterHeading: "a Number",
        search: "",
        suggestions: [],
        recentNews: [],
        featuredGames: [],
        canonicalUrl: `${SITE_URL}/letter/0-9`,
        req: {
          path: "/letter/0-9",
          originalUrl: "/letter/0-9",
          query: {},
          params: { letter: "0-9" },
          session: {},
        },
  ...
  it("heads itself with the name its title gives it", async () => {
    const { document } = new JSDOM(await renderDigits()).window;

    expect(document.querySelector("main h1")?.textContent?.trim()).toBe(
      "MS-DOS Games Starting with a Number",
    );
  });

  it("gives the breadcrumb schema the same name and its own address", async () => {
    const html = await renderDigits();
    const trail = nodeOfType(html, "BreadcrumbList")!;
    const { document } = new JSDOM(html).window;

    expect(
      trail.itemListElement.map((item: { name: string }) => item.name),
    ).toEqual(["Home", "Games starting with a number"]);
    expect(
      document.querySelector('link[rel="canonical"]')?.getAttribute("href"),
    ).toBe(`${SITE_URL}/letter/0-9`);
  });
});
