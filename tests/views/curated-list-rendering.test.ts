...
describe("a curated list page", () => {
  ...
  /**
   * Nothing stands between the page's <h1> and the cards, so their titles are
   * its second level. They were <h3>s — the card partial's default, which is
   * right under a listing grid's own <h2> (views/games/game-filters.ejs) and a
   * skipped level on every curated list. /most-played, which ranks its cards
   * the same way, already wrote them as <h2>s.
   */
  it("titles its cards one level below the page's own heading", async () => {
    const { document } = new JSDOM(await renderPage()).window;
    const levels = [
      ...document.querySelectorAll("main h1, main h2, main h3, main h4"),
    ].map((heading) => Number(heading.tagName.slice(1)));

    expect(levels.slice(0, games.length + 1)).toEqual([
      1,
      ...games.map(() => 2),
    ]);
