...
/**
 * Stands in for middlewares/sidebar-data.ts, which is what fills this local on
 * a real request. Null means "the middleware did not run", which is the case
 * every test here but the two that set it — and the case the route has to
 * treat as "show everything".
 */
let nonEmptyListSlugs: Set<string> | null = null;

app.use((req, res, next) => {
  if (nonEmptyListSlugs) res.locals.nonEmptyListSlugs = nonEmptyListSlugs;
...
describe("Lists Routes", () => {
  ...
  describe("GET /game-lists", () => {
    ...
    /**
     * The route answers 404 for a list with nothing in it, and routes/sitemap.ts
     * already skips one — so this page was the last place handing out links to
     * an address that does not resolve, one card per empty list, on the one
     * page that links every list there is.
     */
    it("leaves out a list the catalogue holds nothing for", async () => {
      nonEmptyListSlugs = new Set(["top-dos-games", "best-rpg-games"]);

      try {
        const response = await request(server).get("/game-lists");

        expect(response.body.data.lists.map((list: any) => list.slug)).toEqual([
          "top-dos-games",
          "best-rpg-games",
        ]);
      } finally {
        nonEmptyListSlugs = null;
      }
    });

    /**
     * The sidebar data is loaded per request and is allowed to fail — see the
     * fallback in middlewares/sidebar-data.ts. A blanked database is not a
     * reason to decide the catalogue is empty and serve a page with no lists
     * on it at all; the links go back to being what they were, and the route
     * behind each of them still answers 404 if it has to.
     */
    it("shows every list when the count is not available", async () => {
      const response = await request(server).get("/game-lists");

      expect(response.body.data.lists.length).toBe(LISTS.length);
      expect(ALL_LIST_SLUGS.size).toBe(LISTS.length);
    });
  ...
  describe("GET /:slug — known list", () => {
    ...
    /**
     * The buttons at the foot of a list were drawn for every related slug
     * whatever the catalogue held, so /best-action-games offered
     * /best-platformer-games and /best-fighting-games on a catalogue with
     * neither — two links to a 404 on the page whose whole purpose is to send
     * a reader to another list. The index and the sidebar had already been
     * taught to skip an empty list; this was the one left.
     */
    it("offers only the related lists that have games in them", async () => {
      nonEmptyListSlugs = new Set([
        "best-action-games",
        "best-shooter-games",
        "top-dos-games",
      ]);
      ...
      try {
        const response = await request(server).get("/best-action-games");

        expect(
          response.body.data.relatedLists.map((list: any) => list.slug),
        ).toEqual(["best-shooter-games", "top-dos-games"]);
      } finally {
        nonEmptyListSlugs = null;
      }
    });

    // As on /game-lists: counts that could not be had are not a reason to
    // take the links away, and each list behind one still answers for itself.
    it("offers every related list when the counts are not available", async () => {
      const response = await request(server).get("/best-action-games");

      expect(
        response.body.data.relatedLists.map((list: any) => list.slug),
      ).toEqual(
        LISTS.find((list) => list.slug === "best-action-games")!.relatedSlugs,
      );
    });
