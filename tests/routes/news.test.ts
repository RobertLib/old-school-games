...
describe("News Routes", () => {
  beforeEach(async () => {
    /**
     * One TRUNCATE, the way tests/setup.ts and tests/routes/comments.test.ts
     * do it.
     *
     * This used to be six DELETEs in hand-maintained foreign-key order
     * followed by two ALTER SEQUENCEs — two lists that had to stay in step
     * with each other and with the schema, and a DELETE refused by a foreign
     * key is a cleanup that quietly did not happen, surfacing later as an
     * assertion failing in an unrelated case. CASCADE follows the keys itself
     * and reaches the ones nothing named here at all ("news_slugs",
     * "game_slugs"), and RESTART IDENTITY resets every sequence in the same
     * statement rather than the two somebody remembered to list.
     *
     ...
     * pool by itself in any case. See tests/models/news.test.ts, where the
     * same pair was removed for the same reasons.
     */
    await pool.query(
  ...
  describe("GET /news", () => {
    ...
    /**
     * A row written past News.create, the kind News.findBySlug sanitises on
     * read for. Its page shows "Hello x"; its excerpt here used to read
     * "Hello alert(2) x body{}", because flattening dropped the tags and
     * kept the code between them. See NON_TEXT_ELEMENTS in
     * utils/html-text.ts.
     */
    it("leaves a script's and a style's source out of the excerpt", async () => {
      await pool.query(
        'INSERT INTO "news" ("title", "slug", "content", "userId") VALUES ($1, $2, $3, $4)',
        [
          "Raw",
      ...
      expect(response.body.locals.news[0].excerpt).toBe("Hello x");
    });
