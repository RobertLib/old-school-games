...
// The three routers with admin routes on them, and the guard itself. Imported
// so the admin list below can be derived from what express actually wired up
// rather than restated by hand — see the comment there.
 ...
 */
/**
 * "/js/ui.js" with the content hash the templates stamp into it — the only
 * version express.static now answers "immutable" for.
 */
function versionedUi(): string {
  const publicDir = path.join(
    path.dirname(fileURLToPath(import.meta.url)),
    "../public",
  );

  return createAssetUrl(publicDir)("/js/ui.js");
}
...
describe("the assembled app", () => {
  ...
  describe("security headers", () => {
    /**
     * Not "no-referrer", helmet's default. That policy also makes a browser
     * send `Origin: null` on every same-origin form post, and hasForeignOrigin
     * in middlewares/csrf.ts then refuses the post as coming from another
     * site — the production-only 403 that made the admin login unusable
     * while every test and every development server kept passing.
     */
    it("sends a Referrer-Policy under which a form post carries its origin", async () => {
    ...
    /**
     * A nonce and a host allowlist in one directive is a nonce in name only.
     *
     * "https://www.googletagmanager.com" and "https://www.google-analytics.com"
     * used to sit beside the nonce here, and googletagmanager.com is the
     * best-known CSP bypass there is: it serves attacker-authored JSONP and
     * container payloads, so an injected <script src> naming it runs as this
     * site and never has to guess the nonce at all. They were redundant into
     * the bargain — the one tag this site loads carries the nonce itself; see
     * the <script async nonce="…"> in views/head.ejs, which the case below
     * holds in place.
     *
     * Only script-src. The GA beacons go out under connect-src and img-src,
     * which are asserted separately: a host there may be *sent to* and can
     * never run as this origin.
     */
    it("allowlists no host in script-src, only the nonce and this origin", async () => {
      const response = await request(server).get("/");
      const scriptSrc =
        /(?:^|;)script-src ([^;]*)/
          .exec(response.headers["content-security-policy"])?.[1]
          ?.split(" ")
          .filter(Boolean) ?? [];
      ...
      for (const source of scriptSrc) {
        expect(
          source,
          `script-src names the host ${source}, which the nonce then counts for nothing against`,
        ).not.toMatch(/^https?:/);
      }
    });

    /**
     * The other half of the same decision, and the reason removing the hosts
     * above does not switch analytics off: the tag is allowed by the nonce.
     * Without the nonce attribute on that tag the directive above would refuse
     * it and the site would simply stop reporting, with nothing but the
     * browser console to say so.
     */
    it("loads the analytics tag under the nonce rather than under a host", async () => {
      const head = await readFile(
        path.join(process.cwd(), "views", "head.ejs"),
        "utf-8",
      );

      // By line rather than by a tag-shaped regular expression: the nonce is
      // written as <%= locals.cspNonce %>, and the "%>" that closes it ends
      // any [^>]* long before the src attribute is reached.
      const gtag = head
        .split("\n")
        .find((line) => /<script\b/.test(line) && line.includes("googletagmanager.com"));

      expect(gtag, "views/head.ejs no longer loads the analytics tag").toBeDefined();
      expect(gtag).toContain("nonce=");
    });

    /**
     * Where a hit actually goes, which is not a place anything runs.
     *
     * These have to stay after the script-src hosts were dropped: gtag.js
     * sends its hits with fetch/sendBeacon (connect-src) and falls back to an
     * image ping (img-src), and a refused beacon is reported nowhere — the
     * numbers simply go quiet.
     */
    it("still lets the analytics beacons out", async () => {
      const csp = (await request(server).get("/")).headers[
     ...
     * document of its own can be served a policy of its own. It is a split of
     * the header, and not by itself of the risk: served from this origin, as
     * it is by default, the frame is same-origin with the page and its
     * sandbox is no boundary (see views/games/game-detail.ejs), so code
     * running under this policy can reach the page anyway. The split becomes
     * isolation with PLAYER_ORIGIN; tests/player-origin.test.ts covers that
     * arrangement, and everything here the default one.
     */
    describe("the player's policy, and only the player's", () => {
      ...
      /**
       * PLAYER_ORIGIN unset is the default, and it means exactly what the
       * site did before the setting existed. These are the places the
       * setting reaches, pinned in their default state: the page frames
       * this origin, the frame is framed by it, nothing about the player's
       * headers moves, and no host is treated as the player's.
       */
      describe("with no player origin of its own", () => {
        it("lets pages frame this origin", async () => {
          expect(await directive("/", "frame-src")).toEqual(["'self'"]);
        });

        it("lets only this origin frame the player", async () => {
          expect(await directive("/js-dos.html", "frame-ancestors")).toEqual([
            "'self'",
          ]);
        });
        ...
        it("keeps helmet's frame header on the player, and opens nothing to CORS", async () => {
          const frame = await request(server).get("/js-dos.html");
          const script = await request(server).get("/js/ui.js");

          expect(frame.headers["x-frame-options"]).toBe("SAMEORIGIN");
          expect(frame.headers["access-control-allow-origin"]).toBeUndefined();
          expect(script.headers["access-control-allow-origin"]).toBeUndefined();
        });

        // No host is the player's, so a request under any other name is
        // answered as it always was — the full site, not the player's gate.
        it("answers under another host as it always has", async () => {
          const response = await request(server)
            .get("/js/ui.js")
            .set("Host", "play.example.test");
    ...
    /**
     * Where a refusal is reported — see routes/csp-report.ts. Neither policy
     * named anywhere to report to, and every directive in this file that was
     * ever wrong was found by somebody opening a console.
     */
    describe("violation reporting", () => {
      ...
      it.each([
        ["/", "a page"],
        ["/js-dos.html", "the player"],
      ])("has %s (%s) report to the endpoint both ways", async (path) => {
        const csp = (await request(server).get(path)).headers[
          "content-security-policy"
        ] as string;

        // report-to for the Reporting API, report-uri for the browsers
        // without it — a browser with it ignores report-uri.
        expect(directiveOf(csp, "report-to")).toEqual(["csp-endpoint"]);
        expect(directiveOf(csp, "report-uri")).toEqual(["/csp-report"]);
      });

      /**
       * report-to is only a group name; without this header a browser that
       * implements the Reporting API has nowhere to send a report, and it
       * ignores report-uri once report-to is present — so the report would
       * go nowhere at all.
       */
      it.each(["/", "/js-dos.html", "/js/ui.js"])(
        "declares where the group sends on %s",
        async (path) => {
          const response = await request(server).get(path);
      ...
      // With helmet rather than after the redirects, like everything else in
      // this block: the redirect onto HTTPS is served a policy too.
      it("declares it on a production redirect too", async () => {
        const previous = process.env.NODE_ENV;
        process.env.NODE_ENV = "production";
  ...
  describe("cache headers", () => {
     ...
     *
     * "no-store" rather than no header at all, which is what this asserted
     * while the route sent nothing: absence is not a refusal — a shared cache
     * with no Cache-Control to read is free to apply a heuristic of its own,
     * and a crawler handed "Sitemap temporarily unavailable" out of a proxy
     * after the outage is over is worse off than one handed it during.
     */
    ...
    it("leaves the asset lifetimes alone", async () => {
      const response = await request(server).get(versionedUi());
  ...
  describe("static assets", () => {
    ...
    it("keeps a versioned asset for a year and a bare one for a day", async () => {
      const versioned = await request(server).get(versionedUi());
    ...
    /**
     * "immutable" is a promise that these bytes are what that address will
     * always hold, and it used to be made to any ?v= at all. During a rolling
     * deploy a page from a new machine names ?v=NEW, the request can land on
     * a machine still serving the old file, and the old bytes went out under
     * the new address for a year. A version this process did not compute is
     * served, but only on "no-cache" terms.
     */
    it("promises nothing lasting for a version it did not compute", async () => {
      const response = await request(server).get("/js/ui.js?v=0000000000");
     ...
     * sandbox withholds what the emulator's ordinary code has no business
     * doing — navigating the top-level page, opening windows, submitting
     * forms. It does not hold back code that means harm while the frame is
     * same-origin with the page: with allow-scripts and allow-same-origin
     * such a frame can lift its own sandbox and read this page, cookies,
     * token and all. That is what PLAYER_ORIGIN is for; see
     * tests/player-origin.test.ts.
     */
    describe("the player frame is sandboxed", () => {
       ...
       * script. The sandbox still withholds navigation and popups from the
       * emulator's ordinary code; see the comment beside the frame for what
       * it does not withhold, and from whom.
       */
      it("keeps it on its origin", async () => {
     ...
     * the player's origin, which the frame's "allow-same-origin" makes
     * possible. Read off the frame's own address, because that origin is
     * this site's by default and PLAYER_ORIGIN when the player has one of
     * its own; written as window.location.origin it silently dropped every
     * message in the second arrangement. The receiving end checks source and
     * origin too; that half is covered in tests/js/js-dos-player.test.ts.
     */
    it("addresses the fullscreen relay to the frame's own origin", async () => {
      const detail = await readFile(
        new URL("../views/games/game-detail.ejs", import.meta.url),
        "utf-8",
      );
      ...
      expect(detail).toContain(
        "postMessage({ action: 'clickFullscreen' }, new URL(iframe.src).origin)",
      );
...
describe("the admin flow through the assembled app", () => {
  ...
  const ADMIN_ROUTES: [string, string][] = [
    ...
    ["POST", "/comments/1/source"],
    ["POST", "/comments/1/source/delete"],
  ];

  /**
   * And the half of the contract the list above cannot keep on its own.
   *
   * "A new admin route belongs on it" is an instruction to a person, and the
   * case it matters in is the one where nobody is thinking about this file:
   * somebody adds `router.post("/:id/publish", isAuth, isAdmin, …)` to
   * routes/news.ts, every route test passes because those apps are always an
   * ADMIN, and the block below goes on driving eleven routes that were already
   * guarded. The suite would report full coverage of a guard it had stopped
   * checking one route of.
   *
   * So the set is also derived, from the routers themselves. Each one's
   * `.stack` is the layers express built out of the `router.get`/`router.post`
   * calls; a layer's `route.stack` is that route's own handler chain, and the
   * question is whether the isAdmin imported here — compared by identity, not
   * by name, because a second function called isAdmin would pass a name check
   * and guard nothing — is in it. The mount paths are the ones app.ts uses:
   * "/games", "/comments" and "/news" (see the comment there about why those
   * three are mounted above homeRoutes).
   *
   * The parameters are substituted with "1" so the derived set is spelled the
   * way ADMIN_ROUTES is: these are addresses the block below actually
   * requests, not patterns.
   */
  type RouteLayer = {
    route?: {
      path: string;
  ...
  function guardedByIsAdmin(mount: string, router: unknown): string[] {
    const { stack } = router as { stack: RouteLayer[] };

    return stack.flatMap((layer) => {
      const route = layer.route;

      // A layer with no `route` is middleware mounted on the router rather
      // than a route of its own; none of these routers has one, and one added
      // later is not an admin route either.
      if (!route) return [];
      if (!route.stack.some((handler) => handler.handle === isAdmin)) return [];

      const substituted = route.path.replace(/:[^/]+/g, "1");
      // `router.post("/")` under a mount of "/games" is POST /games, not
      // POST /games/ — which is what the request below asks for.
      const full = `${mount}${substituted === "/" ? "" : substituted}`;

      return (
        Object.keys(route.methods)
          // express records `router.all()` as "_all"; nothing here uses it.
  ...
  it("has a hand-written admin list that matches the routers", () => {
    const derived = [
      ...guardedByIsAdmin("/games", gamesRoutes),
      ...guardedByIsAdmin("/news", newsRoutes),
      ...guardedByIsAdmin("/comments", commentsRoutes),
    ].sort();

    const declared = ADMIN_ROUTES.map(
      ([method, route]) => `${method} ${route}`,
    ).sort();

    // Both directions at once: a route that grew a guard and is not exercised
    // below, and an entry here for a route that has lost one.
...
/**
 * The NODE_ENV guard, checked the same way and for a failure that is invisible
 * without it.
 *
 * Every environment check in this app is an equality test against
 * "production" or "test", so an unrecognised value is not an error anywhere —
 * it silently selects the development branch of all of them at once. A deploy
 * that set "prod", or "Production", or left a trailing space in a Fly secret,
 * would skip the HTTPS and canonical-host redirects, skip "trust proxy" (so
 * every limiter counts the proxy rather than the visitor), skip the
 * SESSION_SECRET guards above and drop Secure from the session cookie —
 * without a line anywhere saying so.
 */
describe("the NODE_ENV guard", () => {
  async function boot(env: string | undefined): Promise<unknown> {
    const previousEnv = process.env.NODE_ENV;
    const previousSecret = process.env.SESSION_SECRET;
    ...
    // So a value that is accepted does not then fall foul of the production
    // secret guards above, which are a different subject.
  ...
  it.each(["prod", "Production", "production ", "staging", ""])(
    "refuses to boot with NODE_ENV=%o",
    async (env) => {
      await expect(boot(env)).rejects.toThrow(/NODE_ENV/);
    },
  );

  it.each(["development", "production", "test"])(
    "boots with NODE_ENV=%s",
    async (env) => {
      await expect(boot(env)).resolves.toBeDefined();
    },
  );

  // Unset is the documented way of saying "development" — it is what a bare
  // `node index.ts` gives you, and every check in the app already reads it
  // that way.
  it("leaves an unset NODE_ENV alone", async () => {
    await expect(boot(undefined)).resolves.toBeDefined();
  });
});

describe("the assembled app — fixes from the review", () => {
  /**
   * Postgres refuses a NUL in a text parameter (SQLSTATE 22021), and the
   * error carried no status, so it was a 500 with a stack trace at error level
   * — for a "%00" anyone can put in an address as often as they like.
   */
  describe("a NUL byte in the request", () => {
    it("is a 400 when it reaches a query", async () => {
      const error = vi.spyOn(logger, "error");

      try {
        const response = await request(server).get("/?search=doom%00");
    ...
    // The game route refuses anything that is not slug-shaped before any
    // query, so this one never reaches Postgres at all.
    it("is a 404 on the game route, which never asks", async () => {
      const response = await request(server).get("/doom%00");

      expect(response.status).toBe(404);
    });
  });

  // Browsers and crawlers ask for /favicon.ico whatever the <head> says, and
  // it used to be looked up as a game slug twice before it got a full 404.
  it("points /favicon.ico at the icon the site has", async () => {
    const response = await request(server).get("/favicon.ico");

    expect(response.status).toBe(301);
    expect(response.headers.location).toBe("/favicon.png");
  });

  /**
   * express.static redirected a directory to its trailing-slash form and the
   * trailing-slash middleware redirected it straight back — a loop the
   * browser reports as ERR_TOO_MANY_REDIRECTS.
   */
  it.each(["/js", "/css", "/fonts", "/images"])(
    "answers %s rather than redirecting it in a loop",
    async (path) => {
      const response = await request(server).get(path);
  ...
  /**
   * An error raised by a router mounted above where res.locals.req used to be
   * set — the sitemap's, here — reached the 400 view with no `req` local, and
   * finalhandler answered with its own page instead (a stack trace, in
   * development).
   */
  it("renders its own 400 page for a malformed sitemap address", async () => {
    const response = await request(server).get("/sitemap-%E0.xml");

    expect(response.status).toBe(400);
    expect(response.text).toContain("400 - Bad Request");
    expect(response.text).not.toContain("is not defined");
  });

  // GA4's regional collection hosts under analytics.google.com, which the
  // apex entry does not cover.
  it("lets GA4 reach its regional collection hosts", async () => {
    const response = await request(server).get("/");
    const csp = response.headers["content-security-policy"] as string;
    const connect = /(?:^|;)\s*connect-src ([^;]*)/.exec(csp)?.[1] ?? "";

    expect(connect.split(" ")).toContain("https://*.analytics.google.com");
  });

  /**
   * The emulator's relaxed policy used to be keyed on req.path, so every
   * response at that address carried it — including the site's own 403 page
   * for a POST, which express.static does not serve.
   */
  it("keeps the player's policy off anything but the player file", async () => {
    const response = await request(server).post("/js-dos.html");
    const csp = response.headers["content-security-policy"] as string;
  ...
  /**
   * The global limiter used to sit above the access log, so the requests it
   * refused with a 429 — the ones worth seeing during a flood — never left a
   * line. Checked on the source, because provoking the real limit takes a
   * thousand requests and leaves this process's budget spent for every test
   * after it.
   */
  it("logs a request before the global limiter can refuse it", async () => {
    const source = await readFile(
      path.join(path.dirname(fileURLToPath(import.meta.url)), "../app.ts"),
      "utf8",
    );

    const logger = source.indexOf('res.on("finish", () => {');
    const limiter = source.indexOf("app.use(limiter);");

    expect(logger).toBeGreaterThan(-1);
    expect(limiter).toBeGreaterThan(logger);
  });
});
