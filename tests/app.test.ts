...
describe("the assembled app", () => {
  ...
  describe("security headers", () => {
    ...
    it("allows the pinned js-dos CDN everywhere the player needs it", async () => {
      ...
      for (const directive of [
        ...
        const value =
          new RegExp(`(?:^|;)${directive} ([^;]*)`).exec(csp)?.[1] ?? "";
  ...
  describe("static assets", () => {
     ...
     * the frame's Content-Security-Policy is not in the file. helmet writes
     * it onto the response, from a directive list in app.ts, so an immutable
     * copy pins a header that lives somewhere else entirely: the next change
