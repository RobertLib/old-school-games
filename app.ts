...
import { SESSION_SECRET } from "./utils/session-secret.ts";
...
import cspReportRoutes, {
  CSP_REPORT_GROUP,
  CSP_REPORT_PATH,
} from "./routes/csp-report.ts";
...
import { isUnstorableTextError } from "./utils/pg-errors.ts";
...
/**
 * The three values NODE_ENV is allowed to hold, checked before anything reads
 * it.
 *
 * Nothing checked it, and every one of the reads spread through this app is a
 * `=== "production"` or a `=== "test"` — so an unrecognised value is not a
 * configuration error anywhere, it is *silently the development branch of
 * every one of them at once*. A deploy that set NODE_ENV="prod", or "Production",
 * or left a trailing space in a Fly secret, would boot happily and then: skip
 * the HTTPS and canonical-host redirects, skip "trust proxy" (so every rate
 * limiter counts the proxy's address rather than the visitor's), skip the
 * SESSION_SECRET guards below, drop the Secure attribute from the session
 * cookie, and write its logs into files inside a container instead of to the
 * collector. Not one of those says anything at boot; the site simply serves
 * over plain HTTP with a forgeable session cookie.
 *
 * Refusing to start is the right shape here, unlike the deliberately advisory
 * warnings elsewhere (a stale schema, an unreachable Postgres — see /healthz
 * and warnOnPendingMigrations in index.ts). Those leave the process in a state
 * this app is built for; this one leaves it in a state nobody has reasoned
 * about, and the failure is invisible from the outside.
 *
 * Unset is left alone: it is the documented way of saying "development", it is
 * what a bare `node index.ts` gives you, and every check in the app already
 * treats it as such.
 */
...
if (
  nodeEnv !== undefined &&
  !(NODE_ENVIRONMENTS as readonly string[]).includes(nodeEnv)
) {
  throw new Error(
    `NODE_ENV="${nodeEnv}" is not one of ${NODE_ENVIRONMENTS.join(", ")}. ` +
      `Every environment check in this app compares NODE_ENV for equality, so ` +
      `an unrecognised value silently selects the development branch of all of ` +
      `them — no HTTPS redirect, no trust proxy, no session-secret guard.`,
  );
}
...
/**
 * Whether a request is addressed to the player's own origin — PLAYER_ORIGIN in
 * utils/site.ts. Always false while that is unset, which is what keeps the
 * default arrangement exactly as it was: nothing below that asks this question
 * changes anything for a deployment that has not chosen a player origin.
 *
 * The Host header, compared exactly, the way the canonical-host redirect
 * compares it. It is the client's to write, and that is fine here: the answer
 * only ever narrows what a request can get — the player's own files or a 404
 * — so a forged Host buys nothing a visitor could not have by asking the
 * player origin directly.
 */
function isPlayerHost(req: { headers: { host?: string } }): boolean {
  return PLAYER_HOST !== null && req.headers.host === PLAYER_HOST;
}
 ...
 *
 * And then to the top of the stack, directly under the nonce, because a third
 * path had the same hole: the sitemap and the feed routers are mounted above
 * the body parsers, and so above where this used to sit. An error raised in
 * one of them — "/sitemap-%E0.xml" fails to decode its parameter, a 400 —
 * reached the handler at the foot of this file with no `req` local, the 400
 * view threw "req is not defined" in head.ejs, and finalhandler answered
 * instead: a bare page in production, and in development a stack trace with
 * absolute paths and the template's source. Nothing that can render a view
 * is mounted above this line now.
 *
 * Nothing above needs to move with it: this sets two locals and touches no
 * I/O, so running it for every request, static files included, costs
 * nothing. It reads req.query, which express parses itself — no body parser
 * involved — and the session it does not have yet is only ever read through
 * `?.` in the templates.
 */
...
app.use(
  helmet({
    /**
     * Not helmet's default of "no-referrer", which broke every form post in
     * production while leaving development untouched.
     *
     * The Referrer-Policy decides more than the Referer header: it also
     * decides what a browser writes in the Origin header of a same-origin
     * POST. Under "no-referrer" the Fetch standard has it send `Origin: null`
     * — for *every* form submit on the site, the login included — and
     * hasForeignOrigin in middlewares/csrf.ts, which compares the header
     * against the canonical origin, refused each one with the "session has
     * expired" 403. The check runs only under NODE_ENV=production, so a
     * developer's machine never saw it and the site was unusable for an
     * admin without a single failing test.
     *
     * Under this policy a same-origin submit carries the real origin, which
     * is what the check was written to compare against, and a cross-site
     * submit over HTTPS still carries the *other* site's origin, which it
     * still refuses. It is also what browsers ship as their default when no
     * header is sent at all, so nothing about what a third party learns from
     * a click on an outbound link changes: the bare origin, never the path.
     *
     * tests/app.test.ts asserts the header, and tests/middlewares/csrf.test.ts
     * asserts that a null Origin stays refused — because the fix belongs
     * here, not in accepting "null", which is also what a sandboxed frame or
     * a data: URL sends.
     */
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        "connect-src": [
          ...
          // The regional collection hosts under analytics.google.com
          // (region1.analytics.google.com and its siblings), which GA4 sends
          // hits to once Google Signals is on and which Google's own CSP
          // guidance for GA4 lists. The apex above does not cover them — a
          // wildcard never matches the bare host, nor the bare host a
          // subdomain — so those hits were refused silently, with nothing but
          // a console line to show for it.
          "https://*.analytics.google.com",
         ...
         *
         * The relaxed set is no longer in this header — PLAYER_CSP further
         * down replaces it on the one response that runs the emulator. What
         * that buys depends on where the player is served from, and it is
         * less than the split suggests in the default arrangement:
         *
         *   - From this origin (PLAYER_ORIGIN unset), the header is the only
         *     thing that is split. The frame is same-origin with the page and
         *     its sandbox is not a boundary (see views/games/game-detail.ejs),
         *     so code running in the frame can reach this page's DOM, its
         *     CSRF token and its nonce directly — PLAYER_CSP is, in practice,
         *     the policy of the whole origin.
         *   - From an origin of its own (PLAYER_ORIGIN), it is split for
         *     real: the relaxed policy covers a document on the player's
         *     origin, which cannot reach anything on this one.
         */
        "script-src": [
          "'self'",
          ...
          scriptNonce,
          // No host allowlist beside the nonce, deliberately.
          //
          // "https://www.googletagmanager.com" and
          // "https://www.google-analytics.com" used to be listed here, and
          // both were redundant *and* harmful. Redundant because the one tag
          // this site loads from either of them carries the nonce itself —
          // see the <script async nonce="…" src="…/gtag/js?id=…"> in
          // views/head.ejs — so it is allowed by the nonce and needs no host
          // entry. Harmful because googletagmanager.com is the best-known
          // nonce bypass there is: it serves arbitrary attacker-authored
          // JSONP and container payloads, so an injected <script src> naming
          // it runs as this site without ever needing to guess a nonce, and
          // the whole point of minting one per response is lost.
          //
          // The GA *beacons* are a different directive and stay: see
          // "connect-src" and "img-src", which is where a hit is actually
          // sent. Nothing there may run as this origin.
        ],
        ...
        "font-src": ["'self'", "data:"],
        /**
         * The one frame any page here embeds is the player, so this names
         * wherever the player is served from — and only that.
         *
         * With PLAYER_ORIGIN set that is the player origin *instead of*
         * 'self', not as well as it. This origin still has /js-dos.html on
         * disk, and a page able to frame it here would be able to put the
         * emulator back beside the page it was moved away from — through a
         * stale template, a cached page from before the switch, or injected
         * markup. Naming only the player origin means the browser refuses
         * that frame, redirects included, whatever asked for it.
         */
        "frame-src": PLAYER_ORIGIN === null ? ["'self'"] : [PLAYER_ORIGIN],
        /**
         * Where a refusal is reported, so that it is reported somewhere other
         * than a console nobody has open — see routes/csp-report.ts.
         *
         * Both directives, because browsers split on them. report-to names
         * the endpoint group declared in the Reporting-Endpoints header
         * below, and a browser that implements it ignores report-uri when
         * both are present; report-uri is for the ones that do not — Firefox,
         * today — which POST each violation to that address themselves.
         * Relative, so a report goes back to the origin that served the
         * document; the route explains why that matters.
         */
...
/**
 * The endpoint group "report-to" names in both policies, declared on every
 * response.
 *
 * report-to on its own is only a name: without this header a browser that
 * implements the Reporting API has nowhere to send a report and — because it
 * ignores report-uri once report-to is present — sends it nowhere at all,
 * which is the silence the directive was added to end. Every response rather
 * than only the pages, for the reason the redirects give for running helmet
 * first, and because the player's response needs it as much as a page does.
 *
 * Relative, and resolved by the browser against the response that carries
 * it; routes/csp-report.ts explains why that is the point.
 */
app.use((req, res, next) => {
  res.setHeader(
    "Reporting-Endpoints",
...
app.use((req, res, next) => {
  ...
  // The player's own host is the one other host this app answers under on
  // purpose, and it is left alone. Redirected, the frame's address would be
  // sent to this site's /js-dos.html — which frame-src no longer allows once
  // PLAYER_ORIGIN is set, so the player would simply not load — and the
  // HTTPS upgrade below keeps it on its own origin for the same reason.
  const playerHost = isPlayerHost(req);

  if (!playerHost && req.headers.host !== CANONICAL_HOST) {
  ...
  if (forwardedProto !== "https") {
    return res.redirect(
      301,
      `${(playerHost && PLAYER_ORIGIN) || CANONICAL_ORIGIN}${target}`,
    );
  }
...
//
// Kept in a constant as well: express.static below asks the same instance
// which hash a file really has before it promises a year of "immutable".
const asset = createAssetUrl(PUBLIC_DIR);
...
// Where the game page frames the player from: null for this site, which is
// the default, or the origin of its own that PLAYER_ORIGIN names — see
// utils/site.ts, and the player markup in views/games/game-detail.ejs that
// reads it.
app.locals.playerOrigin = PLAYER_ORIGIN;
...
// How a genre is written for a reader — "Action", "RPG" — which a dozen
// templates used to work out for themselves by title-casing the enum, and all
// of them turned "RPG" into "Rpg". See utils/genre-label.ts.
app.locals.genreLabel = genreLabel;
app.locals.genreInSentence = genreInSentence;

// A cover or screenshot stored as a path on this site, made fetchable from
// outside it — og:image, twitter:image and the JSON-LD are read by consumers
// that do not resolve a relative address. See absoluteUrl in utils/site.ts.
...
/**
 * Who may frame the player: the game pages it belongs on, and nothing else.
 *
 * 'self' while the player is served from this site. With a player origin,
 * 'self' would name the player's origin framing itself, and the pages that
 * really frame it are on SITE_URL — so that is named, alone. It is also why a
 * development server cannot frame a player origin: its pages are on
 * localhost, not on SITE_URL. In production the canonical-host redirect is
 * what guarantees every game page is.
 */
 ...
 * is the player's origin, which serves nothing but the player, so this site
 * is named instead. The page resolves such a path against SITE_URL before it
 * hands it over — see the player markup in views/games/game-detail.ejs — and
 * express.static below answers the player's cross-origin fetch for it.
 */
const PLAYER_BUNDLE_SOURCES =
  PLAYER_ORIGIN === null ? [MEDIA_ORIGIN] : [MEDIA_ORIGIN, SITE_URL];
 ...
 *
 * Framing the player is what lets it carry a policy of its own: /js-dos.html
 * is a separate document, so it is served this header while the pages keep
 * the strict one. Whether that also *isolates* it is a different question,
 * and the answer depends on where the frame is served from:
 *
 *   - From this origin — PLAYER_ORIGIN unset, the default — it does not.
 *     The frame is same-origin with the game page, and the sandbox on its
 *     <iframe> carries allow-scripts and allow-same-origin, which together
 ...
 *     relaxed script-src covers nothing but the player.
 *
 * utils/site.ts has the rest of that argument, and why the default is still
 ...
 *
 * Three directives are here for reasons that are easy to miss:
 *
 ...
 *   - "connect-src" naming where games come from, because the bundle is
 *     fetched by the emulator itself. The parent page never fetches it. See
 *     PLAYER_BUNDLE_SOURCES.
 *   - "frame-ancestors", which is the only thing keeping another site from
 *     framing the player — see PLAYER_FRAME_ANCESTORS.
 *
...
const PLAYER_CSP = [
  ...
  "object-src 'none'",
  // Reported, like the page's policy and for its reason — see report-to
  // there. The addresses are relative on purpose: the player's reports go
  // back to whichever origin served it, and on a player origin that endpoint
  // is the one thing it answers for besides the player's own files.
  `report-to ${CSP_REPORT_GROUP}`,
  `report-uri ${CSP_REPORT_PATH}`,
].join("; ");

// A name of its own rather than mounted inline, because it is mounted twice:
// once for this site, and once behind the player origin's gate below, which
// hands it the few requests that origin answers.
const serveStatic = express.static(PUBLIC_DIR, {
  maxAge: "1d",
  /**
   * Off, because the trailing-slash middleware above undoes it. With the
   * default, express.static answers a request for a *directory* —
   * "/js", "/css", "/fonts", "/images" — with a 301 to the same path plus
   * "/", and the middleware that strips trailing slashes sends "/js/"
   * straight back to "/js": a redirect loop the browser reports as
   * ERR_TOO_MANY_REDIRECTS. Nothing here serves a directory index, so the
   * request now simply falls through to the ordinary 404.
   */
  redirect: false,
  setHeaders(res, filePath) {
    const onPlayerHost = isPlayerHost(res.req);

    /**
     * Swaps helmet's policy for the player's on the one file that needs it.
     *
     * Overwritten rather than appended — setHeader, not appendHeader — and
     * this runs after helmet has written its own header. Two
     * Content-Security-Policy headers on one response are not a choice
     * between policies: the browser enforces the intersection, which would
     * refuse the emulator exactly as the strict policy does.
     *
     * Here, keyed on the file express.static is actually sending, rather
     * than in a middleware keyed on req.path, which is where this used to
     * be. The path matched every response at that address, not only the
     * file: "POST /js-dos.html" is not served by express.static at all, and
     * the 403 page the CSRF check renders for it went out under the
     * emulator's relaxed policy — 'unsafe-eval' and all — on a document
     * that is the site's own layout. setHeaders runs only when the file is
     * the answer, for a 200 and for the 304 that revalidates it alike, so
     * the "?v=" the template stamps in and the "?stream=" the player reads
     * are matched without being looked at.
     *
     * The frame is revalidated rather than cached for a day (see
     * REVALIDATED_FILES below), which matters here: a copy of the frame
     * frozen in a browser would be a copy of this policy frozen with it.
     *
     * With a player origin, only there. This origin still has the file and
     * still answers for it, but under the site's own policy, where js-dos
     * is refused before it can start. That is the HTML standard's other
     * warning about sandboxing: it is "of minimal help if an attacker can
     * convince the user to just visit the hostile content directly" — and
     * a /js-dos.html here under the relaxed policy would be exactly that,
     * one link from running the emulator beside an admin's session, frame
     * or no frame. Keyed on the file, so "/%6As-dos.html" and every other
     * spelling that express.static decodes to the same file are covered.
     */
    if (
      filePath === PLAYER_FRAME &&
      (PLAYER_ORIGIN === null || onPlayerHost)
    ) {
      res.setHeader("Content-Security-Policy", PLAYER_CSP);

      // helmet's "X-Frame-Options: SAMEORIGIN" comes off the player's own
      // origin, because same-origin is the one thing its parent never is
      // there. A browser that implements frame-ancestors ignores the header
      // once the policy names that directive; one that knew only the header
      // would refuse the frame outright, and no value of it can name
      // another origin. Left alone on this origin, where it is true.
      if (onPlayerHost) res.removeHeader("X-Frame-Options");
    }

    // A game stored as a path on this site, fetched by a player on its own
    // origin. js-dos asks for the bundle with fetch(), which is a CORS
    // request once it is cross-origin, and without this header the browser
    // hands the emulator a network error instead of the game. Everything
    // in public/ is public and nothing here varies by who asks, so one
    // fixed origin in the header gives the player nothing any visitor
    // could not fetch — and with no Access-Control-Allow-Credentials, no
    // cookie of this site's rides along.
    if (PLAYER_ORIGIN !== null && !onPlayerHost) {
      res.setHeader("Access-Control-Allow-Origin", PLAYER_ORIGIN);
    }
    ...
    if (REVALIDATED_FILES.has(filePath)) {
      ...
      // Content-Security-Policy is not part of the file — the branch just
      // above writes PLAYER_CSP onto the response — so a frozen copy
    ...
    } else if (res.req?.query?.v !== undefined) {
      // A request carrying the file's own content hash came from a
      // template that stamped it into the address, so that copy can never
      // go stale and is worth keeping for a year. Anything asked for by its
      // bare name keeps the modest lifetime above.
      //
      // *The file's own* hash, compared rather than assumed. This used to
      // grant "immutable" to any ?v= at all — style.css?v=0000000000 was
      // kept for a year — and the case that matters is not a hand-typed
      // one. During a rolling deploy the machines disagree about what the
      // file is: a page from a new machine names ?v=NEW, the request can
      // land on a machine still running the old release, and the old bytes
      // went out under the new address with a year's promise attached. A
      // version this process did not compute is therefore answered
      // "no-cache": the browser uses what it got and asks again next time,
      // by which point a machine that has the right file answers.
      res.setHeader(
        "Cache-Control",
        res.req.query.v === asset.versionOf(filePath)
...
/**
 * Where both policies send a refusal — see routes/csp-report.ts.
 *
 * This high up for three reasons, each a thing below it would do to a
 * report. validateCsrf would refuse every one, because a browser sends a
 * report with no token and no way to get one. The session and the CSRF
 * middleware would hand each one a cookie it will never send back. And the
 * global limiter would count them against the visitor's budget for the
 * site, so a page whose policy refuses something on every view could spend
 * a reader's allowance on reports and lock them out of the pages. The
 * endpoint has a limiter of its own.
 *
 * Above the access log too: each report is already a line of its own, and a
 * refused flood logging one line per refusal would be the flood the
 * endpoint's limiter exists to keep out of the log.
 *
 * And above the player origin's gate, which answers nothing else there: the
 * player's reports go back to the origin that served it.
 */
app.use(cspReportRoutes);

/**
 * The player origin answers for the player, and for nothing else.
 *
 * A second hostname pointed at this app would otherwise be a second copy of
 ...
 * spelling some other file fall through to the 404.
 *
 * Where it sits is deliberate. Below the redirects, which know this host —
 * the canonical-host redirect leaves it alone, and the trailing-slash
 * redirect's Location is relative, so it stays on it — and below compression,
 * so the player's files are compressed as they are here. /healthz stays above
 * all of it: the platform's check is addressed to the machine, and a health
 * check must never depend on which host it named.
 *
 * With PLAYER_ORIGIN unset isPlayerHost is always false, and every request
 * goes straight on to express.static as it always has.
 */
...
// The icon lives at /favicon.png, which every page names in its <head>, but
// "/favicon.ico" is what browsers ask for on their own — for a feed, a JSON
// response, a bookmark — and what every crawler probes. It used to fall all
// the way through to the game route, which looked "favicon.ico" up as a slug
// and then in the slug history before rendering the full 404 page: two
// queries and a page render for an icon, on one of the most-requested
// addresses a site has. Permanent, so a browser asks once.
...
//
// Ahead of the global limiter too, for the same reason one step further up:
// mounted below it, the requests the limiter refused with a 429 never reached
// this, so during a flood — the one time the refusals are the story — the log
// showed a quiet site.
app.use((req, res, next) => {
...
// Below the access log, so a refusal is logged like any other answer — see
// the comment on the logger just above.
app.use(limiter);
...
/**
 * The session store, kept in a name of its own rather than constructed inline.
 *
 * connect-pg-simple underneath, but not its set(): see utils/session-store.ts
 * for why a save must never re-create a row the logout or a password reset
 * has deleted, which the library's upsert did whenever a request of that
 * session was still in flight.
 *
 * connect-pg-simple starts a timer the moment it is built: it prunes expired
 * rows on an interval of its own, indefinitely, and nothing ever stopped it.
 * Two things came of that. A drain (utils/shutdown.ts) closed the listener and
 * ended the pool while this timer was still scheduled, so a prune firing in
 * between ran a query against a pool that had gone — logged through errorLog
 * as a failure on every shutdown that happened to land on one. And under test
 * the timer is a handle nothing releases, which is a runner that does not exit
 * on its own.
 *
 * It is put on app.locals below so index.ts can hand it to the drain without
 * importing anything from here beyond the app itself, which is the same
 * arrangement every other cross-file value in this app uses.
 */
...
const sessionOptions: SessionOptions = {
  ...
  // needs to be seen. See sessionStore above for why it is built there.
  store: sessionStore,
  // The environment's, or random bytes per process outside production — see
  // utils/session-secret.ts, which is also where the keys derived from it
  // live. The guards above have already refused a missing or weak one in
  // production.
...
// For index.ts, which hands it to the drain so the store's prune timer is
// stopped before the pool it queries is ended — see sessionStore above and
// closeStore in utils/shutdown.ts. On app.locals rather than exported, so a
 ...
 *
 * One error carries no status and is still the client's: Postgres refusing a
 * string it cannot store, which in practice is a NUL byte sent as "%00". See
 * isUnstorableTextError for why that was a 500 with a stack trace until now.
 */
function clientErrorStatus(error: unknown): number | null {
  if (typeof error !== "object" || error === null) return null;

  if (isUnstorableTextError(error)) return 400;
