    ...
    contentSecurityPolicy: {
      directives: {
        ...
        "script-src": [
          ...
          scriptNonce,
          // js-dos compiles the emulator at runtime and cannot work without
          // this. Unlike 'unsafe-inline' it is not reachable by injecting
          // markup, so the trade is worth making.
          "'unsafe-eval'",
          // Pinned to one release and hash-checked — see JS_DOS_SOURCE above.
          JS_DOS_SOURCE,
          "https://www.googletagmanager.com",
          "https://www.google-analytics.com",
          "blob:",
         ...
         *
         * Named explicitly because the fallback chain does not reach anything
         * that would allow it: an undeclared "worker-src" falls back to
         * "child-src", and an undeclared "child-src" to "default-src", which
         * is "'self'" alone. js-dos does not load its worker from a URL on
         * this origin — emulators.js fetches wdosbox.js from the pinned
         * release, turns the response into a blob, and constructs the worker
         * from the resulting "blob:" address:
         *
         ...
         * browsers have differed, which is why "script-src" above already
         * carries an explicit blob: for the same kind of address. Leaving
         * this directive to a fallback that does not was the asymmetry: the
         * worker is the part that runs the game, and a refused worker is
         * reported nowhere but the browser console.
         *
         * JS_DOS_SOURCE is here as well, so a js-dos that ever loads the
         * worker by URL rather than through a blob is covered too.
         */
 ...
 *
 * The pool waits up to ten seconds for a free connection (see db.ts), and a
 * health check that hangs that long is a failed one — the single outcome this
 * endpoint is written to avoid while the database is away.
 */
  ...
  express.static(PUBLIC_DIR, {
    ...
    setHeaders(res, filePath) {
      if (REVALIDATED_FILES.has(filePath)) {
        ...
        // Content-Security-Policy is not part of the file — helmet writes it
        // onto the response — so a frozen copy freezes a header that lives
        // in app.ts, and the next change to this policy would reach those
        // browsers a year late. Revalidating answers with a 304 that carries
        // today's header, which is exactly what is wanted.
...
// Before the sidebar, so a request that falls on a check boundary reads its
// widgets from caches another machine's write has already emptied.
