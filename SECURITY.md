...
- **The emulator running third-party code.** js-dos executes DOS binaries by
  design. The bundles come from `MEDIA_ORIGIN` or from a path on this site —
  nothing else — which the player enforces, the admin form's validation
  refuses to store otherwise, and the Content-Security-Policy names. The
  player's frame is only an isolation boundary when it is served from its own
  origin (`PLAYER_ORIGIN`, see the README); on the site's own origin its
  sandbox can be undone from inside, so a finding that depends on that is
  known, not new.
