...
router.post("/", validateComment, commentRateLimit, async (req, res, next) => {
  try {
    ...
    //
    // The test is the content type rather than utils/expects-json.ts, which
    // answers "does this endpoint report its errors as JSON" (yes, for both
    // paths) and not "did this particular request come from fetch".
    //
