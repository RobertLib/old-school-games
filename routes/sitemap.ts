...
let sitemapGeneratedAt: number | undefined;
...
router.get("/sitemap-index.xml", async (req, res) => {
  ...
  try {
    ...
    add({ url: `/how-to-play`, changefreq: "monthly", priority: 0.7 });
    ...
    // Get games and news with minimal data for sitemap
    const [games, newsItems] = await Promise.all([
      Game.findForSitemap(),
      News.findForSitemap(),
    ]);

    // Compute lastmod for listing pages from most recently updated content
    ...
    const homeLastmod =
      latestGameDate && latestNewsDate
        ? new Date(Math.max(latestGameDate, latestNewsDate))
            .toISOString()
            .split("T")[0]
        : latestGameDate
          ? new Date(latestGameDate).toISOString().split("T")[0]
          : undefined;
    ...
    entries.unshift(
      ...
      { url: `/`, changefreq: "daily", priority: 1.0, lastmod: homeLastmod },
    );
