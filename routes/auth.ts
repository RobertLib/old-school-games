...
router.post("/login", loginLimiter, accountLimiter, async (req, res, next) => {
  ...
  req.session.regenerate((err) => {
    ...
    // minting tokens the now-privileged session accepts. Rotating it here
    // costs nothing: the redirect below renders no page, so no token masking
    // the old secret is left anywhere.
