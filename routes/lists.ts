...
// There is no lastmod for a curated list any more, and so no listLastmod
// beside countListGames. It dated a list by the newest games.updatedAt in its
// filter, but a list is a ranking by ratings, which that column never
// reflects, so the date described a different page from the one served. The
// sitemap now leaves the element off; routes/sitemap.ts says why.

/**
 * The slugs of the lists that actually hold something, for every place that
 * links to a list rather than serving one: views/lists/lists-index.ejs, the
 * "Top Lists" block in views/left-sidebar.ejs, the related-list buttons at the
 * foot of every list (see the route below) and the no-results panel in
 * views/games/game-list.ejs.
 *
 * The route below answers 404 for an empty list and routes/sitemap.ts already
 * skips one. This comment used to say the index and the sidebar "were the only
 * places left advertising an address that answers 404", and they were not:
 * every curated list went on offering buttons for its related lists whatever
 * they held — /best-action-games linked /best-platformer-games and
 * /best-fighting-games on a catalogue with neither — and the chrome linked
 * every genre in the enum and all twenty-six letters the same way. All of
 * those read the same counts now, through the one cached entry in
 * middlewares/sidebar-data.ts (loadNonEmptyListings), so a link to a listing
 * is drawn only when the page behind it exists. A link to a 404 from the
 * chrome of a site is worse than a sitemap entry: every crawl follows it, from
 * everywhere.
 *
 * Counted the same way the sitemap counts, through countListGames over one
 * GROUP BY, rather than fifteen Game.count() calls — see the comment there.
 * It takes the counts rather than asking for them because the middleware's
 * entry answers several questions from that one query, and caching it there
 * alongside the rest of the chrome is what keeps this off the per-request
 * path.
 */
export function listSlugsWithGames(
  counts: Map<string, number>,
  totalGames: number,
): Set<string> {
  return new Set(
    LISTS.filter((list) => countListGames(list, counts, totalGames) > 0).map(
      (list) => list.slug,
    ),
  );
}
...
/**
 * What every caller above falls back to when that query fails.
 *
 * Every list, deliberately: a blanked database is not a reason to decide the
 * catalogue is empty and take the whole "Top Lists" block off every page. The
 * worst case of guessing wrong is the link that was already there, and the
 * route behind it answers 404 for it exactly as it did before.
 */
export const ALL_LIST_SLUGS: ReadonlySet<string> = new Set(
...
router.get("/game-lists", (req, res) => {
  // Off res.locals rather than queried here: middlewares/sidebar-data.ts has
  // already loaded and cached this for the sidebar that every page of the site
  // renders, and asking again would be a second copy of the same GROUP BY on
  // the one page that renders both.
  const nonEmpty = (res.locals.nonEmptyListSlugs ??
    ALL_LIST_SLUGS) as ReadonlySet<string>;
  ...
  res.render("lists/lists-index", {
    ...
    // A card for a list the route answers 404 for is an invitation to a dead
    // end, and this page is the one place the whole set of them is linked.
    lists: LISTS.filter((list) => nonEmpty.has(list.slug)),
  });
});
...
router.get("/:slug", async (req, res, next) => {
  ...
  // Only the related lists that have something in them. These buttons were
  // drawn for every slug in relatedSlugs whatever the catalogue held, so
  // /best-action-games offered /best-platformer-games and
  // /best-fighting-games on a catalogue with neither, and both answer 404 —
  // on the one kind of page whose whole job is to send a reader to another
  // list. Off res.locals, like /game-lists above, because the middleware has
  // already counted every list for the sidebar; every list when it could not.
  const nonEmpty = (res.locals.nonEmptyListSlugs ??
    ALL_LIST_SLUGS) as ReadonlySet<string>;
  ...
  const relatedLists = list.relatedSlugs
    ...
    .filter(
      (related): related is ListDefinition =>
        related !== undefined && nonEmpty.has(related.slug),
    );
