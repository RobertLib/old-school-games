...
(function () {
  ...
  function getCurrentTheme() {
    ...
    // Only a theme this file knows; anything else is read as the default.
    // A name that is not in THEMES — one since retired, or a value something
    // else on this origin wrote under the key — used to be passed on as it
    // was: no option in the menu matched it, so updateDropdownActiveState
  ...
  function initDropdownState() {
    document.querySelectorAll(".dropdown").forEach((dropdown) => {
      ...
      toggle.addEventListener("click", () => {
        if (isOpen()) {
          // Closing from the toggle leaves focus on the toggle. This used to
          // blur whatever in the dropdown had focus — and when that was the
          // toggle itself, which it is for anyone who shuts the menu with
          // Enter or Space, focus fell to <body> and the next Tab started
          // again from the top of the page. Only focus inside the menu has
          // to move, because the item it is on is about to be hidden (and a
          // browser drops focus on a hidden element to <body> by itself);
          // it goes to the toggle that was just pressed, as it does on
          // Escape. Focus anywhere else is left where the visitor put it.
          close(Boolean(menu && menu.contains(document.activeElement)));
        } else {
          open();
        }
      });
