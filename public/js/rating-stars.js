...
class RatingStars extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Not this.title: that is HTMLElement's own property, the tooltip
    // attribute, and a title="" on the element would hang a tooltip over
    // the stars. See labelStars for what it is for.
    this.gameTitle = (this.getAttribute("data-title") || "").trim();
  ...
  /**
   * Names the stars after the game they rate.
   *
   * Every star said "Rate 3 out of 5" and nothing more, so a listing page —
   * a card per game, five stars on each — gave a screen reader two dozen
   * identical sets: tabbing through them, or pulling up the page's buttons
   * as a list (VoiceOver's rotor, NVDA's elements list), there was no
   * telling which game a star belonged to. The title arrives on data-title,
   * which every view that renders an interactive widget passes — the same
   * attribute public/js/favorites.js names the heart by.
   *
   * It goes into each star's own label, because a list of buttons shows them
   * one at a time and out of context, and onto a role="group" around the
   * five, which is what is announced on the way in. The role also makes the
   * aria-disabled that setStarsBusy puts on this element a valid one; ARIA
   * does not support it on a <span> with no role.
   *
   * Set with setAttribute after the markup is in, never interpolated into
   * it: the title is data from the database, and one with a quote in it
   * would close the attribute and turn the rest of itself into markup.
   *
   * Without a title the stars keep the labels they always had. Read-only
   * stars — the carousel, where the whole card is already a link — are not
   * controls and carry no labels to qualify.
   */
  labelStars() {
    if (this.readonly || !this.gameTitle) return;

    const group = this.shadowRoot.querySelector(".stars");

    if (group) {
      group.setAttribute("role", "group");
      group.setAttribute("aria-label", `Rate ${this.gameTitle}`);
    }

    this.shadowRoot.querySelectorAll(".star").forEach((star, index) => {
      // The number first, as in the untitled label, so "Doom 2" can never
      // read as the rating.
      star.setAttribute(
        "aria-label",
        `Rate ${index + 1} out of 5 for ${this.gameTitle}`,
      );
    });
  }
